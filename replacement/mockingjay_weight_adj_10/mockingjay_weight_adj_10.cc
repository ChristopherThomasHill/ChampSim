...
#include "champsim.h"
#include "mockingjay_weight_adj_10.h"

bool mockingjay_weight_adj_10::is_sampled_set(long set)
{
...
uint64_t mockingjay_weight_adj_10::CRC_HASH(uint64_t _blockAddress)
{
...
uint64_t mockingjay_weight_adj_10::build_signature(uint32_t triggering_cpu, champsim::address ip, access_type type, uint8_t hit)
{
  uint64_t signature;
  if (NUM_CPUS == 1) 
  {
...
uint64_t mockingjay_weight_adj_10::get_sampled_cache_index(uint64_t full_addr)
{
...
uint64_t mockingjay_weight_adj_10::get_sampled_cache_tag(uint64_t x)
{
...
uint64_t mockingjay_weight_adj_10::get_prefetch_sampled_cache_index(uint64_t full_addr)
{
  full_addr = full_addr >> LOG2_BLOCK_SIZE;
...
uint64_t mockingjay_weight_adj_10::get_prefetch_sampled_cache_tag(uint64_t x)
{
...
int mockingjay_weight_adj_10::search_sampled_cache(uint64_t blockAddress, bool metadata, uint32_t set)
{
...
int mockingjay_weight_adj_10::search_prefetch_sampled_cache(uint64_t blockAddress, uint32_t set)
{
...
void mockingjay_weight_adj_10::detrain(uint32_t set, int way, bool metadata)
{
...
void mockingjay_weight_adj_10::prefetch_detrain(uint32_t set, int way)
{
...
int mockingjay_weight_adj_10::temporal_difference(int init, int sample)
{
  if (sample > init) {
...
int mockingjay_weight_adj_10::increment_timestamp(int input)
{
  input++;
...
int mockingjay_weight_adj_10::time_elapsed(int global, int local)
{
  if (global >= local) {
    return global - local;
  }
...
int mockingjay_weight_adj_10::prefetch_time_elapsed(int local)
{
...
double mockingjay_weight_adj_10::calculate_value(bool metadata, int time_remaining, double accuracy)
{
  if (metadata)
  {
    double slope;
    if (accuracy > 0.875) slope = METADATA_HISTORY_RATIO / 21.0;
    else if (accuracy > 0.75) slope = METADATA_HISTORY_RATIO / 17.5;
    else if (accuracy > 0.625) slope = METADATA_HISTORY_RATIO / 14.0;
    else if (accuracy > 0.5) slope = METADATA_HISTORY_RATIO / 10.5;
    else if (accuracy > 0.375) slope = METADATA_HISTORY_RATIO / 7.0;
    else if (accuracy > 0.25) slope = METADATA_HISTORY_RATIO / 3.5;
    else if (accuracy > 0.125) slope = METADATA_HISTORY_RATIO / 1.0;
    else slope = METADATA_HISTORY_RATIO * 3.5;
...
mockingjay_weight_adj_10::mockingjay_weight_adj_10(CACHE* _cache) : mockingjay_weight_adj_10(_cache, _cache->NUM_SET, _cache->NUM_WAY) {}

mockingjay_weight_adj_10::mockingjay_weight_adj_10(CACHE* _cache, long sets, long ways) 
                    : replacement(_cache),
                      cache(_cache),
                      NUM_SET(sets),
                      NUM_WAY(ways),
...
long mockingjay_weight_adj_10::find_victim(uint32_t triggering_cpu, uint64_t instr_id, long set, const champsim::cache_block* current_set, champsim::address ip, champsim::address full_addr, access_type type)
{
...
void mockingjay_weight_adj_10::update_replacement_state(uint32_t triggering_cpu, long set, long way, champsim::address full_addr, champsim::address ip, champsim::address victim_addr, access_type type, uint8_t hit, const std::shared_ptr<champsim::MetadataRequest>& meta_request, bool local_pref)
{
...
void mockingjay_weight_adj_10::replacement_final_stats()
{
  reuse_profiler.print_profiler(this);
  mockingjay_profiler.print_profiler(this);
}

void mockingjay_weight_adj_10::ReuseProfiler::track_info(long set, champsim::address full_addr, champsim::address ip, access_type type, bool hit)
{
...
void mockingjay_weight_adj_10::ReuseProfiler::print_profiler(mockingjay_weight_adj_10* parent)
{
...
void mockingjay_weight_adj_10::MockingjayProfiler::record_bypass(champsim::address ip, access_type type)
{
  if (type == access_type::WRITE || type == access_type::TRANSLATION) return;
...
void mockingjay_weight_adj_10::MockingjayProfiler::record_update(long set, long way, champsim::address ip, champsim::address victim_addr, access_type type, bool hit, int insert_etr, int current_etr)
{
...
void mockingjay_weight_adj_10::MockingjayProfiler::print_profiler(mockingjay_weight_adj_10* parent)
{
