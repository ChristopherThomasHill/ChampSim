...
#include "champsim.h"
#include "mockingjay_split_6.h"

bool mockingjay_split_6::is_sampled_set(long set)
{
...
uint64_t mockingjay_split_6::CRC_HASH(uint64_t _blockAddress)
{
...
uint64_t mockingjay_split_6::build_signature(uint32_t triggering_cpu, champsim::address ip, access_type type, uint8_t hit)
{
  uint64_t signature;
  if (NUM_CPUS == 1) 
  {
...
uint64_t mockingjay_split_6::get_sampled_cache_index(uint64_t full_addr)
{
...
uint64_t mockingjay_split_6::get_sampled_cache_tag(uint64_t x)
{
...
int mockingjay_split_6::search_sampled_cache(uint64_t blockAddress, bool metadata, uint32_t set)
{
...
void mockingjay_split_6::detrain(uint32_t set, int way)
{
...
int mockingjay_split_6::temporal_difference(int init, int sample)
{
  if (sample > init) {
...
int mockingjay_split_6::increment_timestamp(int input)
{
  input++;
...
int mockingjay_split_6::time_elapsed(int global, int local)
{
   if (global >= local) {
      return global - local;
    }
...
mockingjay_split_6::mockingjay_split_6(CACHE* cache) : mockingjay_split_6(cache, cache->NUM_SET, cache->NUM_WAY) {}

mockingjay_split_6::mockingjay_split_6(CACHE* cache, long sets, long ways) 
                    : replacement(cache),
                      NUM_SET(sets),
                      NUM_WAY(ways),
                      METADATA_WAYS(6),
...
long mockingjay_split_6::find_victim(uint32_t triggering_cpu, uint64_t instr_id, long set, const champsim::cache_block* current_set, champsim::address ip, champsim::address full_addr, access_type type)
{
  uint32_t min_way;
  uint32_t max_way;
...
void mockingjay_split_6::update_replacement_state(uint32_t triggering_cpu, long set, long way, champsim::address full_addr, champsim::address ip, champsim::address victim_addr, access_type type, uint8_t hit)
{
