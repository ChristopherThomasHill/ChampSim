...
#include "champsim.h"
#include "mockingjay_weight_adj_3.h"

bool mockingjay_weight_adj_3::is_sampled_set(long set)
{
...
uint64_t mockingjay_weight_adj_3::CRC_HASH(uint64_t _blockAddress)
{
...
uint64_t mockingjay_weight_adj_3::build_signature(uint32_t triggering_cpu, champsim::address ip, access_type type, uint8_t hit)
{
  uint64_t signature;
  if (NUM_CPUS == 1) 
  {
...
uint64_t mockingjay_weight_adj_3::get_sampled_cache_index(uint64_t full_addr)
{
...
uint64_t mockingjay_weight_adj_3::get_sampled_cache_tag(uint64_t x)
{
...
uint64_t mockingjay_weight_adj_3::get_prefetch_sampled_cache_index(uint64_t full_addr)
{
  full_addr = full_addr >> LOG2_BLOCK_SIZE;
...
uint64_t mockingjay_weight_adj_3::get_prefetch_sampled_cache_tag(uint64_t x)
{
...
int mockingjay_weight_adj_3::search_sampled_cache(uint64_t blockAddress, bool metadata, uint32_t set)
{
...
int mockingjay_weight_adj_3::search_prefetch_sampled_cache(uint64_t blockAddress, uint32_t set)
{
...
void mockingjay_weight_adj_3::detrain(uint32_t set, int way, bool metadata)
{
...
void mockingjay_weight_adj_3::prefetch_detrain(uint32_t set, int way)
{
...
int mockingjay_weight_adj_3::temporal_difference(int init, int sample)
{
  if (sample > init) {
...
int mockingjay_weight_adj_3::increment_timestamp(int input)
{
  input++;
...
int mockingjay_weight_adj_3::time_elapsed(int global, int local)
{
  if (global >= local) {
    return global - local;
  }
...
int mockingjay_weight_adj_3::prefetch_time_elapsed(int local)
{
...
mockingjay_weight_adj_3::mockingjay_weight_adj_3(CACHE* _cache) : mockingjay_weight_adj_3(_cache, _cache->NUM_SET, _cache->NUM_WAY) {}

mockingjay_weight_adj_3::mockingjay_weight_adj_3(CACHE* _cache, long sets, long ways) 
                    : replacement(_cache),
                      cache(_cache),
                      NUM_SET(sets),
                      NUM_WAY(ways),
...
long mockingjay_weight_adj_3::find_victim(uint32_t triggering_cpu, uint64_t instr_id, long set, const champsim::cache_block* current_set, champsim::address ip, champsim::address full_addr, access_type type)
{
  ...
  for (uint32_t way = 0; way < NUM_WAY; way++) {
    ...
    else 
    {
      ...
      if (way_acc > 0.875) way_etr = comparable_etr / 8.0; // 14+ Useful (Rest Pollution)
      else if (way_acc > 0.75) way_etr = comparable_etr / 6.5; // 12+ Useful (Rest Pollution)
      else if (way_acc > 0.625) way_etr = comparable_etr / 5.0; // 10+ Useful (Rest Pollution)
      else if (way_acc > 0.5) way_etr = comparable_etr / 3.5; // 8+ Useful (Rest Pollution)
      else if (way_acc > 0.375) way_etr = comparable_etr / 2.0; // 6+ Useful (Rest Pollution)
      else if (way_acc > 0.25) way_etr = comparable_etr / 0.5; // 4+ Useful (Rest Pollution)
  ...
  if (type == access_type::METADATA_STORE)
  {
    ...
    double insert_etr;
    if (insert_acc > 0.875) insert_etr = comparable_etr / 8.0; // 14+ Useful (Rest Pollution)
    else if (insert_acc > 0.75) insert_etr = comparable_etr / 6.5; // 12+ Useful (Rest Pollution)
    else if (insert_acc > 0.625) insert_etr = comparable_etr / 5.0; // 10+ Useful (Rest Pollution)
    else if (insert_acc > 0.5) insert_etr = comparable_etr / 3.5; // 8+ Useful (Rest Pollution)
    else if (insert_acc > 0.375) insert_etr = comparable_etr / 2.0; // 6+ Useful (Rest Pollution)
    else if (insert_acc > 0.25) insert_etr = comparable_etr / 0.5; // 4+ Useful (Rest Pollution)
...
void mockingjay_weight_adj_3::update_replacement_state(uint32_t triggering_cpu, long set, long way, champsim::address full_addr, champsim::address ip, champsim::address victim_addr, access_type type, uint8_t hit, const std::shared_ptr<champsim::MetadataRequest>& meta_request, bool local_pref)
{
...
void mockingjay_weight_adj_3::replacement_final_stats()
{
  reuse_profiler.print_profiler(this);
  mockingjay_profiler.print_profiler(this);
}

void mockingjay_weight_adj_3::ReuseProfiler::track_info(long set, champsim::address full_addr, champsim::address ip, access_type type, bool hit)
{
...
void mockingjay_weight_adj_3::ReuseProfiler::print_profiler(mockingjay_weight_adj_3* parent)
{
...
void mockingjay_weight_adj_3::MockingjayProfiler::record_bypass(champsim::address ip, access_type type)
{
  if (type == access_type::WRITE || type == access_type::TRANSLATION) return;
...
void mockingjay_weight_adj_3::MockingjayProfiler::record_update(long set, long way, champsim::address ip, champsim::address victim_addr, access_type type, bool hit, int insert_etr, int current_etr)
{
...
void mockingjay_weight_adj_3::MockingjayProfiler::print_profiler(mockingjay_weight_adj_3* parent)
{
