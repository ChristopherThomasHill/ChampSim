...
#include "champsim.h"
#include "mockingjay_quarter_metadata.h"

bool mockingjay_quarter_metadata::is_sampled_set(long set)
{
...
uint64_t mockingjay_quarter_metadata::CRC_HASH(uint64_t _blockAddress)
{
...
uint64_t mockingjay_quarter_metadata::build_signature(uint32_t triggering_cpu, champsim::address ip, access_type type, uint8_t hit)
{
  uint64_t signature;
  if (NUM_CPUS == 1) 
  {
...
uint64_t mockingjay_quarter_metadata::get_sampled_cache_index(uint64_t full_addr, bool metadata)
{
...
uint64_t mockingjay_quarter_metadata::get_sampled_cache_tag(uint64_t x)
{
...
int mockingjay_quarter_metadata::search_sampled_cache(uint64_t blockAddress, bool metadata, uint32_t set)
{
...
void mockingjay_quarter_metadata::detrain(uint32_t set, int way)
{
...
int mockingjay_quarter_metadata::temporal_difference(int init, int sample)
{
  if (sample > init) {
...
int mockingjay_quarter_metadata::increment_timestamp(int input)
{
  input++;
...
int mockingjay_quarter_metadata::time_elapsed(int global, int local)
{
   if (global >= local) {
      return global - local;
    }
...
mockingjay_quarter_metadata::mockingjay_quarter_metadata(CACHE* cache) : mockingjay_quarter_metadata(cache, cache->NUM_SET, cache->NUM_WAY) {}

mockingjay_quarter_metadata::mockingjay_quarter_metadata(CACHE* cache, long sets, long ways) 
                    : replacement(cache),
                      NUM_SET(sets),
                      NUM_WAY(ways),
...
long mockingjay_quarter_metadata::find_victim(uint32_t triggering_cpu, uint64_t instr_id, long set, const champsim::cache_block* current_set, champsim::address ip, champsim::address full_addr, access_type type)
{
  uint32_t min_way;
  uint32_t max_way;
...
void mockingjay_quarter_metadata::update_replacement_state(uint32_t triggering_cpu, long set, long way, champsim::address full_addr, champsim::address ip, champsim::address victim_addr, access_type type, uint8_t hit)
{
  ...
  if (is_sampled_set(set))
  {
    uint64_t sampled_cache_index = get_sampled_cache_index(full_addr.to<uint64_t>(), (type == access_type::METADATA_STORE) || (type == access_type::METADATA_LOAD));
    uint64_t sampled_cache_tag = get_sampled_cache_tag(full_addr.to<uint64_t>());
    int sampled_cache_way = search_sampled_cache(sampled_cache_tag, type == access_type::METADATA_STORE || type == access_type::METADATA_LOAD, sampled_cache_index);
    ...
    for (int w = 0; w < SAMPLED_CACHE_WAYS; w++) {
      if (sampled_cache[sampled_cache_index][w].valid == false) {
        ...
        sampled_cache[sampled_cache_index][w].metadata = type == access_type::METADATA_STORE || type == access_type::METADATA_LOAD;
