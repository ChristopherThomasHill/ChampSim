#ifndef REPLACEMENT_MOCKINGJAY_QUARTER_METADATA_H
#define REPLACEMENT_MOCKINGJAY_QUARTER_METADATA_H
...
class mockingjay_quarter_metadata : public champsim::modules::replacement {
  const long NUM_SET;
  const long NUM_WAY;
  ...
  uint64_t get_sampled_cache_index(uint64_t full_addr, bool metadata);
...
public:
  ...
  explicit mockingjay_quarter_metadata(CACHE* cache);
  mockingjay_quarter_metadata(CACHE* cache, long sets, long ways);
