...
#include "champsim.h"
#include "mockingjay_weight_adj_13.h"

bool mockingjay_weight_adj_13::is_sampled_set(long set)
{
...
uint64_t mockingjay_weight_adj_13::CRC_HASH(uint64_t _blockAddress)
{
...
uint64_t mockingjay_weight_adj_13::build_signature(uint32_t triggering_cpu, champsim::address ip, access_type type, uint8_t hit)
{
  uint64_t signature;
  if (NUM_CPUS == 1) 
  {
...
uint64_t mockingjay_weight_adj_13::get_sampled_cache_index(uint64_t full_addr)
{
...
uint64_t mockingjay_weight_adj_13::get_sampled_cache_tag(uint64_t x)
{
...
uint64_t mockingjay_weight_adj_13::get_prefetch_sampled_cache_index(uint64_t full_addr)
{
  full_addr = full_addr >> LOG2_BLOCK_SIZE;
...
uint64_t mockingjay_weight_adj_13::get_prefetch_sampled_cache_tag(uint64_t x)
{
...
int mockingjay_weight_adj_13::search_sampled_cache(uint64_t blockAddress, bool metadata, uint32_t set)
{
...
int mockingjay_weight_adj_13::search_prefetch_sampled_cache(uint64_t blockAddress, uint32_t set)
{
...
void mockingjay_weight_adj_13::detrain(uint32_t set, int way, bool metadata)
{
...
void mockingjay_weight_adj_13::prefetch_detrain(uint32_t set, int way)
{
...
int mockingjay_weight_adj_13::temporal_difference(int init, int sample)
{
  if (sample > init) {
...
int mockingjay_weight_adj_13::increment_timestamp(int input)
{
  input++;
...
int mockingjay_weight_adj_13::time_elapsed(int global, int local)
{
  if (global >= local) {
    return global - local;
  }
...
int mockingjay_weight_adj_13::prefetch_time_elapsed(int local)
{
...
double mockingjay_weight_adj_13::calculate_value(bool metadata, int time_remaining, double accuracy)
{
  if (metadata)
  {
    double slope;
    if (accuracy > 0.875) slope = METADATA_HISTORY_RATIO / 24.0;
    else if (accuracy > 0.75) slope = METADATA_HISTORY_RATIO / 20.0;
    else if (accuracy > 0.625) slope = METADATA_HISTORY_RATIO / 16.0;
    else if (accuracy > 0.5) slope = METADATA_HISTORY_RATIO / 12.0;
    else if (accuracy > 0.375) slope = METADATA_HISTORY_RATIO / 8.0;
    else if (accuracy > 0.25) slope = METADATA_HISTORY_RATIO / 4.0;
    else if (accuracy > 0.125) slope = METADATA_HISTORY_RATIO / 1.0;
    else slope = METADATA_HISTORY_RATIO * 4.0;
  
    return std::abs(slope * static_cast<double>(time_remaining)) + 0.5 * slope;
  }
  else
  {
    return std::abs(static_cast<double>(time_remaining)) + 0.5;
  }
}

mockingjay_weight_adj_13::mockingjay_weight_adj_13(CACHE* _cache) : mockingjay_weight_adj_13(_cache, _cache->NUM_SET, _cache->NUM_WAY) {}

mockingjay_weight_adj_13::mockingjay_weight_adj_13(CACHE* _cache, long sets, long ways) 
                    : replacement(_cache),
                      cache(_cache),
                      NUM_SET(sets),
                      NUM_WAY(ways),
...
long mockingjay_weight_adj_13::find_victim(uint32_t triggering_cpu, uint64_t instr_id, long set, const champsim::cache_block* current_set, champsim::address ip, champsim::address full_addr, access_type type)
{
  ...
  for (uint32_t way = 0; way < NUM_WAY; way++) {
    double way_value;
    if (etr[set][way] == -1 * INF_ETR)
    {
      way_value = 9000.0;
    }
    else if (etr[set][way] == INF_ETR)
    {
      way_value = 8000.0;
    }
    else if (metadata_sig[set][way] == METADATA_NO_SIG)
    {
      way_value = calculate_value(false /*not metadata*/, etr[set][way]);
    }
    else if (!accuracy_hits.count(metadata_sig[set][way])) 
    {
      way_value = 7000.0;
    }
    else 
    {
      ...
      way_value = calculate_value(true /*metadata*/, etr[set][way], way_acc);
    }

    if (way_value > max_value)
    {
      max_value = way_value;
      victim_way = way;
    }
  ...
  {
    if (!accuracy_hits.count(pc_signature))
    {
    ...
    double insert_value = calculate_value(true /*metadata*/, rdp[pc_signature] / GRANULARITY, insert_acc);
  ...
  else
  {
    if (type != access_type::WRITE && rdp.count(pc_signature) &&
            (rdp[pc_signature] > MAX_RD || calculate_value(false /*not metadata*/, rdp[pc_signature] / GRANULARITY) > max_value))
    {
      mockingjay_profiler.record_bypass(ip, type);
      return NUM_WAY;
    }
...
void mockingjay_weight_adj_13::update_replacement_state(uint32_t triggering_cpu, long set, long way, champsim::address full_addr, champsim::address ip, champsim::address victim_addr, access_type type, uint8_t hit, const std::shared_ptr<champsim::MetadataRequest>& meta_request, bool local_pref)
{
  ...
  if (way < NUM_WAY) {
    ...
    } else
    {
      if (metadata_access)
      {
        if ((GRANULARITY - 1 - ((metadata_etr_clock[set]) / METADATA_HISTORY_RATIO)) <= (rdp[signature] % GRANULARITY)) 
          etr[set][way] = (rdp[signature] / GRANULARITY) + 1;
        else 
          etr[set][way] = (rdp[signature] / GRANULARITY);

        etr[set][way] = std::min(etr[set][way], INF_ETR);
      }
      else
      {
        etr[set][way] = (rdp[signature] / GRANULARITY);
...
void mockingjay_weight_adj_13::replacement_final_stats()
{
  reuse_profiler.print_profiler(this);
  mockingjay_profiler.print_profiler(this);
}

void mockingjay_weight_adj_13::ReuseProfiler::track_info(long set, champsim::address full_addr, champsim::address ip, access_type type, bool hit)
{
...
void mockingjay_weight_adj_13::ReuseProfiler::print_profiler(mockingjay_weight_adj_13* parent)
{
...
void mockingjay_weight_adj_13::MockingjayProfiler::record_bypass(champsim::address ip, access_type type)
{
  if (type == access_type::WRITE || type == access_type::TRANSLATION) return;
...
void mockingjay_weight_adj_13::MockingjayProfiler::record_update(long set, long way, champsim::address ip, champsim::address victim_addr, access_type type, bool hit, int insert_etr, int current_etr)
{
...
void mockingjay_weight_adj_13::MockingjayProfiler::print_profiler(mockingjay_weight_adj_13* parent)
{
