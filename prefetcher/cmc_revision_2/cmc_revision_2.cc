...
#include "cmc_revision_2.h"

#include <iostream>

bool cmc_revision_2::Recorder::train_entry(champsim::block_number block_addr)
{
  if (index == 0)
  {
    // first entry
...
void cmc_revision_2::Recorder::reset()
{
    index = 0;
    entries.clear();
}

champsim::address cmc_revision_2::metadata_addr(champsim::address ip, champsim::block_number block_addr)
{
  return champsim::address(block_addr.to<uint64_t>() << LOG2_BLOCK_SIZE);
}

void cmc_revision_2::prefetcher_initialize()
{
  cache = this->intern_;
  recorder = new Recorder(degree);
}

uint32_t cmc_revision_2::prefetcher_cache_operate(champsim::address addr, champsim::address ip, uint8_t cache_hit, bool useful_prefetch, access_type type, uint32_t metadata_in, bool late_prefetch, bool prefetch_from_this)
{
...
void cmc_revision_2::prefetcher_metadata_request_fill(const std::shared_ptr<champsim::MetadataRequest>& request, std::shared_ptr<champsim::MetadataBlk>& blk)
{
...
void cmc_revision_2::prefetcher_metadata_request_update(const std::shared_ptr<champsim::MetadataRequest>& request, std::shared_ptr<champsim::MetadataBlk>& blk, bool hit)
{
  CMCRequest& cmc_request = *static_cast<CMCRequest*>(request.get());
